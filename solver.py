...
class Solver(object):
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            for u in range(len(prior)):
                series_loss += (torch.mean(my_kl_loss(series[u], (prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,
                                                                                               self.win_size)).detach())) + torch.mean(
                    my_kl_loss((prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,self.win_size)).detach(),series[u])))
    ...
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.thre_loader):
            ...
            cri = cri.detach().cpu().numpy()
            attens_energy.append(cri)
            test_data.append(torch.cat([x.view(-1) for x in input_data]))
